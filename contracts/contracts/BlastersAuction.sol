...
contract BonklerAuction is OwnableUpgradeable {
     ...
     */
    uint256 public constant MAX_AUCTIONS = 1000;
    ...
    function initialize(
        ...
        require(
            bonklers != address(0),
            "Bonklers must not be the zero address."
        );
    ...
    function _createAuction(uint256 generationHash) internal returns (bool) {
        ...
        require(bonklerId > MAX_SUPPLY, "No more auctions, reached max supply.");

        // Mint the Bonkler.
        bonklerId = IBlastersNFT(_auctionData.bonklers).mint(generationHash);
    ...
    function _settleAuction() internal {
        ...
        IBonklerNFT(bonklers).transferPurchasedBonkler{value: bonklerShares}(
            bonklerId,
            bidder
        );
